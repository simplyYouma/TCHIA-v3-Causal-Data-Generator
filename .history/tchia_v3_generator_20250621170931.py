...
class CausalCoherentGenerator:
    ...
    def generate_coherent_row(
        ...
        management = self.farmer_model.adjust_management(
    ...
    def _simulate_crop_cycle(
        ...
        # FIX: S'assurer que total_effective n'est pas 0
        if total_effective == 0 and climate_base["SeasonRainfall_mm"] > 0:
            # Fallback : au moins 30% de la pluie est effective
            total_effective = climate_base["SeasonRainfall_mm"] * 0.3
            total_runoff = climate_base["SeasonRainfall_mm"] * 0.7
        
        return {
            "cycle_duration": cycle_days,
            "harvest_date": current_date,
            "total_effective_rain": round(total_effective, 1),
            "total_runoff": round(total_runoff, 1),
            "runoff_percent": round(total_runoff / (total_effective + total_runoff) * 100, 1)
                if (total_effective + total_runoff) > 0 else 0
        }
    ...
    def _calculate_coherent_yield(
        ...
        """
        Calcule rendement cohérent avec toute l'histoire
        
        ✅ Intègre : stress accumulés, décisions, potentiel scénario
        """
        
        # Potentiel de base selon zone et scénario
        yield_range = self.crop_profiles.get_yield_range(crop, zone)
        base_potential = yield_range[1]  # Potentiel max de la zone
        
        # Appliquer le facteur scénario DIRECTEMENT
        scenario_adjusted_potential = base_potential * scenario.yield_potential
        
        # Pour scénario exceptionnel, garantir haut potentiel
        if scenario.type == ScenarioType.EXCEPTIONAL:
            scenario_adjusted_potential = base_potential * 0.95  # 95% du max
        elif scenario.type == ScenarioType.CATASTROPHIC:
            scenario_adjusted_potential = base_potential * 0.1   # 10% du max
        
        # Réductions causales
        # 1. Stress accumulés (plafonné selon scénario)
        if scenario.type == ScenarioType.EXCEPTIONAL:
            stress_factor = max(0.9, 1 - crop_state.accumulated_stress * 0.1)
        elif scenario.type == ScenarioType.CATASTROPHIC:
            stress_factor = min(0.3, 1 - crop_state.accumulated_stress * 0.5)
        else:
            stress_factor = max(0.3, 1 - crop_state.accumulated_stress * 0.3)
        
        # 2. Croissance réduite
        growth_factor = max(0.5, 1 - crop_state.growth_reduction * 0.5)
        
        # 3. Gestion (impact modéré)
        management_bonus = (
            management["fertilizer_adjustment"] * 0.10 +  # Max +10%
            management["density_adjustment"] * 0.05       # Max +5%
        )
        management_factor = 1 + management_bonus
        
        # 4. Suffisance hydrique
        water_needs = self.crop_profiles.get_stress_thresholds(crop)["rainfall_mm"]
        water_ratio = cycle_data["total_effective_rain"] / water_needs
        
        # Ajuster selon scénario
        if scenario.type == ScenarioType.EXCEPTIONAL:
            water_factor = min(1.0, 0.8 + water_ratio * 0.2)  # Moins sensible
        elif scenario.type == ScenarioType.CATASTROPHIC:
            water_factor = min(0.5, water_ratio * 0.5)  # Très sensible
        else:
            water_factor = min(1.0, water_ratio)
        
        # Calcul final avec tous les facteurs
        actual_yield = scenario_adjusted_potential * stress_factor * growth_factor * management_factor * water_factor
        
        # Bornes finales selon scénario
        if scenario.type == ScenarioType.EXCEPTIONAL:
            actual_yield = max(base_potential * 0.75, actual_yield)  # Min 75% du potentiel
        elif scenario.type == ScenarioType.CATASTROPHIC:
            actual_yield = min(base_potential * 0.15, actual_yield)  # Max 15% du potentiel
        
        # Validation finale
        actual_yield = max(50, min(yield_range[1], actual_yield))
        
        # Calcul perte réelle
        loss_percent = ((scenario_adjusted_potential - actual_yield) / scenario_adjusted_potential * 100) if scenario_adjusted_potential > 0 else 0
        
        # Biomasse totale
        harvest_index = 0.35 if crop in ["mil", "sorgho"] else 0.45
        biomass_total = actual_yield / harvest_index
        
        return {
            "potential_yield": round(scenario_adjusted_potential, 1),
            "final_yield": round(actual_yield, 1),
            "loss_percent": round(loss_percent, 1),
            "biomass_total": round(biomass_total, 1),
            "grain_straw_ratio": harvest_index
        }
