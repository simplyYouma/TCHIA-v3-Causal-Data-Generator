...
class CausalCoherenceTests:
    ...
    def test_ndvi_biomass_consistency(row: pd.Series) -> Tuple[bool, str, str]:
        ...
        # Harvest Index avec tolérances scientifiques (±2%)
        expected_hi = {
            "mil": (0.20, 0.37),      # Tolérance +2% au max
            "sorgho": (0.25, 0.47),   # Tolérance +2% au max
            "maïs": (0.40, 0.57),     # Tolérance +2% au max
            "coton": (0.15, 0.27),    # Tolérance +2% au max
            "riz": (0.45, 0.57)       # Tolérance +2% au max
        }
        ...
        if biomass > 0:
            ...
            if not (hi_range[0] <= actual_hi <= hi_range[1]):
                return False, f"Harvest Index hors tolérances ({actual_hi:.3f}) pour {crop} (attendu: {hi_range[0]:.2f}-{hi_range[1]:.2f})", "HIGH"
        
        # Cohérence NDVI-rendement (validation croisée)
        if ndvi_peak > 0.8 and grain_yield < 800:  # Seuil ajusté Mali
            return False, f"NDVI élevé ({ndvi_peak:.2f}) incompatible avec faible rendement ({grain_yield:.0f} kg/ha)", "HIGH"
        
        if ndvi_peak < 0.25 and grain_yield > 2500:  # Seuil ajusté Mali  
            return False, f"NDVI faible ({ndvi_peak:.2f}) incompatible avec rendement élevé ({grain_yield:.0f} kg/ha)", "HIGH"
