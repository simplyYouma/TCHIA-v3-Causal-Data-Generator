...
class CropState:
    ...
    def add_stress_event(self, stress_type: str, intensity: float, duration_days: int):
        """
        Enregistre stress avec accumulation réaliste (CORRIGÉ)
        
        📚 Source : Index stress ICRISAT Mali (0=stress complet, 1=pas stress)
        """
        
        # 🔧 CORRECTION : Facteurs d'impact ajustés selon données Mali
        stage_sensitivity = {
        ...
        # Impact pondéré par semaine (au lieu de par jour)
        impact = intensity * sensitivity * (duration_days / 10)  # Diviseur 10 au lieu de 7
        
        # 🔧 CORRECTION : Accumulation plus modérée
        self.accumulated_stress = self.accumulated_stress * 0.95 + impact * 0.8  # Facteur 0.8 au lieu de 1.0
        
        # Réduction croissance plus progressive
        self.growth_reduction = min(0.8, self.growth_reduction + impact * 0.06)  # 0.06 au lieu de 0.1
...
class CausalCoherentGenerator:
    ...
    def _simulate_crop_cycle(
        ...
        # 🔧 CORRECTION : Définir stage_durations (manquant)
        stage_durations = {
        ...
        for stage_name, duration_frac in stage_durations.items():
            ...
            # 🔧 CORRECTION : Gestion robuste des mois manquants
            if month_fr in monthly_rainfall:
                monthly_rain = monthly_rainfall[month_fr]
            else:
                # Chercher par première lettre ou utiliser défaut
                monthly_rain = 0
                for m in monthly_rainfall:
                    if m[0] == month_fr[0]:
                        ...
                        break
                if monthly_rain == 0:
                    # Utiliser une valeur par défaut basée sur la saison
                    if month_fr in ["mai", "juin", "juillet", "août", "septembre"]:
            ...
            # 🔧 CORRECTION : Calcul ruissellement avec validation
            if rain_per_stage > 0:
        ...
        # 🔧 CORRECTION CRITIQUE : Garantir pluie effective proportionnelle
        total_seasonal_rain = climate_base["SeasonRainfall_mm"]
        
        if total_effective <= 0 and total_seasonal_rain > 0:
            # Fallback intelligent selon scénario
            if scenario.type == ScenarioType.EXCEPTIONAL:
                effectiveness_ratio = 0.6  # 60% efficace
            elif scenario.type == ScenarioType.GOOD:
                effectiveness_ratio = 0.5  # 50% efficace
            elif scenario.type == ScenarioType.AVERAGE:
                effectiveness_ratio = 0.4  # 40% efficace
            elif scenario.type == ScenarioType.DIFFICULT:
                effectiveness_ratio = 0.3  # 30% efficace
            else:  # CATASTROPHIC
                effectiveness_ratio = 0.2  # 20% efficace
            
            total_effective = total_seasonal_rain * effectiveness_ratio
            total_runoff = total_seasonal_rain * (1 - effectiveness_ratio)
