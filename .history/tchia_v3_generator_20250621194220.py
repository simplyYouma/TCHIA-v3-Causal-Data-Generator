...
class ScenarioGenerator:
    ...
    """
    @classmethod
    def _generate_scenario_with_stress_constraints(cls, scenario_type, zone):
        """
        CORRECTION: Contraindre stress selon scénario
        
        CETTE MÉTHODE DOIT REMPLACER LES MÉTHODES _generate_*_scenario
        """
        
        # CORRECTION: Contraintes stress par scénario selon validation
        stress_constraints = {
            "exceptional": {"max_intensity": 0.3, "max_duration": 5, "max_accumulated": 0.5},
            "good": {"max_intensity": 0.5, "max_duration": 10, "max_accumulated": 0.8}, 
            "average": {"max_intensity": 0.6, "max_duration": 15, "max_accumulated": 1.2},
            "difficult": {"max_intensity": 0.8, "max_duration": 20, "max_accumulated": 2.0},  # CORRECTION
            "catastrophic": {"max_intensity": 1.0, "max_duration": 30, "max_accumulated": 3.0}
        }
        
        constraints = stress_constraints[scenario_type.value]
        
        # Génération stress contraints
        stress_timeline = []
        
        stages = ["germination", "vegetative", "flowering", "grain_filling"]
        
        for stage in stages:
            # CORRECTION: Intensité limitée par scénario
            max_intensity = constraints["max_intensity"]
            intensity = np.random.uniform(0, max_intensity)
            
            # CORRECTION: Durée limitée par scénario  
            max_duration = constraints["max_duration"]
            duration = np.random.randint(0, max_duration + 1)
            
            stress_timeline.append({
                "stage": stage,
                "water_stress": intensity,
                "temp_stress": intensity * 0.7,  # Stress thermique corrélé mais plus faible
...
class CropState:
    ...
    def add_stress_event(self, stress_type: str, intensity: float, duration_days: int):
        ...
        stage_sensitivity = {
            ...
            "vegetative": {"water": 0.9, "temp": 0.6},      # CORRECTION: Augmenté de 0.8/0.6
            "flowering": {"water": 2.2, "temp": 1.8},       # CORRECTION: Augmenté de 1.8/1.4
            "grain_filling": {"water": 1.4, "temp": 1.0}    # CORRECTION: Augmenté de 1.0/0.8
        }
        ...
        base_impact = intensity * sensitivity * (duration_days / 8)  # CORRECTION: 8 au lieu de 10
        ...
        self.accumulated_stress = self.accumulated_stress * 0.92 + base_impact  # CORRECTION: 0.92 au lieu de 0.95
...
class CausalCoherentGenerator:
    ...
    def _calculate_coherent_yield(
        ...
        Cette fonction doit REMPLACER la méthode existante dans CausalCoherentGenerator
        SANS CHANGER LE NOM (compatibilité)
        """
        ...
        # CORRECTION: Utiliser string au lieu d'enum pour compatibilité
        ...
        # CORRECTION: Facteurs de réduction ajustés
        stress_reduction = min(0.7, crop_state.accumulated_stress * 0.35)  # CORRECTION: Plus sévère
        stress_factor = max(0.3, 1 - stress_reduction)
        
        growth_factor = max(0.5, 1 - crop_state.growth_reduction * 0.4)     # CORRECTION: Plus sévère
        
        # Facteur hydrique corrigé selon culture
        water_needs = self.crop_profiles.get_stress_thresholds(crop)["rainfall_mm"]
        effective_rain = cycle_data["total_effective_rain"]
        
        # CORRECTION: Facteurs hydriques spécifiques Mali
        if crop == "mil":
            water_factor = min(1.0, effective_rain / max(250, water_needs * 0.5))
        elif crop == "sorgho":
            water_factor = min(1.0, effective_rain / max(350, water_needs * 0.6))
        elif crop == "maïs":
            water_factor = min(1.0, effective_rain / max(500, water_needs * 0.8))
        else:
        ...
        # CORRECTION: Validation finale des pertes
        ...
        if crop_state.accumulated_stress > 0.5:
            hi *= max(0.80, 1 - crop_state.accumulated_stress * 0.12)
