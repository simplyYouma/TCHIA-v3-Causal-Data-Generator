...
class CausalCoherenceTests:
    ...
    def test_stress_yield_coherence(row: pd.Series) -> Tuple[bool, str, str]:
        ...
        # CORRECTION: Seuils ajustés selon recherches Mali validées
        ...
        if accumulated_stress < 0.3 and yield_loss > 50:  # CORRECTION: Seuil ajusté
    ...
    def test_scenario_consistency(row: pd.Series) -> Tuple[bool, str, str]:
        ...
        expected_ranges = {
            "exceptional": {"stress": (0, 0.5), "loss": (0, 15)},     # Années très bonnes
            "good": {"stress": (0.2, 0.8), "loss": (10, 30)},        # Bonnes années
            "average": {"stress": (0.5, 1.2), "loss": (25, 50)},     # Années moyennes  
            "difficult": {"stress": (1.0, 2.0), "loss": (45, 75)},   # Années difficiles
            "catastrophic": {"stress": (1.5, 3.0), "loss": (70, 95)} # Années catastrophiques
        }
