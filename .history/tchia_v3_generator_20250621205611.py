...
class CausalCoherentGenerator:
    ...
    def _calculate_coherent_yield(
        ...
        # CORRECTION SCIENTIFIQUE : Seuils DSSAT-CERES calibrés Mali
        accumulated_stress = crop_state.accumulated_stress

        # Application seuils DSSAT adaptés conditions Mali
        if accumulated_stress > 3.0:
            # Stress catastrophique: 75-90% perte
            stress_loss_percent = max(75, min(90, 60 + accumulated_stress * 15))
        elif accumulated_stress > 2.0:
            # Stress très sévère: 50-75% perte (conforme DSSAT)
            stress_loss_percent = max(50, min(75, 35 + accumulated_stress * 20))
        elif accumulated_stress > 1.2:
            # Stress sévère: 35-50% perte (conforme DSSAT végétatif)
            stress_loss_percent = max(35, min(50, 20 + accumulated_stress * 25))
        elif accumulated_stress > 0.6:
            # Stress modéré: 15-35% perte
            stress_loss_percent = max(15, min(35, accumulated_stress * 30))
        else:
            # Stress faible: 0-15% perte
            stress_loss_percent = max(0, min(15, accumulated_stress * 20))
