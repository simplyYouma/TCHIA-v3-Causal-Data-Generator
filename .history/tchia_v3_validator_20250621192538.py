...
class RealDistributions:
    ...
    STRESS_FREQUENCIES = {
        # 🔧 CORRECTION : Fréquences ajustées selon données Mali
        "water_stress": {"none": 0.10, "mild": 0.30, "moderate": 0.40, "severe": 0.20},  # Plus de stress au Mali
        "temp_stress": {"none": 0.25, "mild": 0.35, "moderate": 0.30, "severe": 0.10}   # Plus de stress thermique
    }
...
class CausalCoherenceTests:
    ...
    def test_stress_yield_coherence(row: pd.Series) -> Tuple[bool, str, str]:
        ...
        if accumulated_stress > 1.0 and yield_loss < 35:
            return False, f"Stress modéré-élevé ({accumulated_stress:.2f}) mais perte faible ({yield_loss:.1f}%)", "CRITICAL"
