...
class CausalCoherenceTests:
    ...
    @staticmethod
    def test_ndvi_biomass_consistency_fixed(row):
        """Test corrigé pour Harvest Index"""
        ndvi_peak = row['NDVI_peak']
        biomass = row['BiomassTotal_kg_ha']
        grain_yield = row['ActualYield_kg_ha']
        crop = row['FinalCrop']
        
        # Harvest Index réaliste avec TOLÉRANCES
        expected_hi = {
            "mil": (0.20, 0.35),
            "sorgho": (0.25, 0.45),
            "maïs": (0.40, 0.55),
        ...
        if biomass > 0:
            ...
            # CORRECTION: Ajouter une tolérance de 0.01
            if not (hi_range[0] - 0.01 <= actual_hi <= hi_range[1] + 0.01):
