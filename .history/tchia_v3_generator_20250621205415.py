...
class HydrologicalModel:
    ...
    """
    CORRECTION SCIENTIFIQUE : Garantie corrélation positive pluie-rendement
    ...
    # CORRECTION : Efficience base selon culture et conditions Mali
    base_efficiency = {
        "mil": 0.45,      # Culture sahélienne tolérante
        "sorgho": 0.40,   # Référence IER : efficience 0.96 kg/ha/mm
        "maïs": 0.35,     # Plus exigeant
        "coton": 0.30,    # Très exigeant
        "riz": 0.50       # Conditions optimales
    }
    
    efficiency = base_efficiency.get(crop, 0.40)
    
    # Ajustement selon conditions sol/humidité
    if soil_moisture < 30:
        efficiency *= 0.85  # Sol sec, moins d'infiltration
    elif soil_moisture > 70:
        efficiency *= 1.15  # Sol humide, meilleure rétention
    
    # Ajustement selon texture sol
    texture_factors = {
        "sableux": 0.90,    # Drainage rapide
        "limoneux": 1.00,   # Équilibré
        "argileux": 1.10    # Meilleure rétention
    }
    
    soil_base = soil_texture.split("-")[0].lower()
    efficiency *= texture_factors.get(soil_base, 1.0)
    
    # CORRECTION CRITIQUE : Contraintes physiques strictes
    efficiency = np.clip(efficiency, 0.20, 0.75)  # Bornes physiques réalistes
    
    # Calcul pluie effective et ruissellement
    effective_mm = rainfall_mm * efficiency
    runoff_mm = rainfall_mm * (1 - efficiency)
    
    # VALIDATION SPÉCIALE : Début saison Mali (règle 30-70% ruissellement)
    if month in ["mai", "juin"] and 15 <= rainfall_mm <= 45:
        runoff_percent_target = np.random.uniform(30, 70)
        runoff_mm = rainfall_mm * runoff_percent_target / 100
        effective_mm = rainfall_mm - runoff_mm
    
    # GARANTIE FINALE : Corrélation positive obligatoire
    # Plus de pluie → Plus de pluie effective (toujours)
...
class CropState:
    ...
    def add_stress_event(self, stress_type: str, intensity: float, duration_days: int):
        ...
        # CORRECTION: Impact plus sévère selon littérature Mali
        base_impact = intensity * sensitivity * (duration_days / 6)  # CORRIGÉ: 6 au lieu de 8
        
        # CORRECTION: Effet non-linéaire renforcé pour stress sévères
        if intensity > 0.6:  # Seuil abaissé
            severity_multiplier = 1 + (intensity - 0.6) * 3  # Facteur augmenté
            base_impact *= severity_multiplier
        
        # CORRECTION: Accumulation encore plus agressive
        self.accumulated_stress = self.accumulated_stress * 0.85 + base_impact  # CORRIGÉ: 0.85 au lieu de 0.92
...
class CausalCoherentGenerator:
    ...
    def _calculate_coherent_yield(
        ...
        # CORRECTION CRITIQUE: Relation directe stress → perte selon DSSAT
        accumulated_stress = crop_state.accumulated_stress
        
        # Application seuils scientifiques validés DSSAT
        if accumulated_stress > 1.8:
            # Stress très sévère: 60-85% perte
            stress_loss_percent = max(60, min(85, 50 + accumulated_stress * 20))
        elif accumulated_stress > 1.2:
            # Stress sévère: 45-60% perte  
            stress_loss_percent = max(45, min(60, 30 + accumulated_stress * 25))
        elif accumulated_stress > 0.8:
            # Stress modéré-élevé: 30-45% perte
            stress_loss_percent = max(30, min(45, 20 + accumulated_stress * 30))
