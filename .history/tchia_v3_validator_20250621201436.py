...
class CausalCoherenceTests:
    ...
    def test_stress_yield_coherence(row: pd.Series) -> Tuple[bool, str, str]:
        ...
        # 🔧 CORRECTION : Seuils basés sur recherche Mali
        # Si accumulated_stress > 1.5, perte attendue > 50% (stress sévère)
        if accumulated_stress > 1.5 and yield_loss < 50:
            return False, f"Stress sévère ({accumulated_stress:.2f}) mais perte insuffisante ({yield_loss:.1f}%)", "CRITICAL"
        
        # Si accumulated_stress > 1.0, perte attendue > 35% (stress modéré)
        if accumulated_stress > 1.0 and yield_loss < 35:
            return False, f"Stress modéré-élevé ({accumulated_stress:.2f}) mais perte insuffisante ({yield_loss:.1f}%)", "CRITICAL"
        
        # Si stress faible mais perte élevée
        if accumulated_stress < 0.3 and yield_loss > 60:
