...
class CausalCoherenceTests:
    ...
    @staticmethod
    def test_stress_yield_coherence(row) -> Tuple[bool, str, str]:
        """
        ...
        # CORRECTION: Relation stress-perte plus sévère
        if accumulated_stress > 2.5:
            # Stress très sévère: 65-88% perte
            stress_loss_percent = max(65, min(88, 40 + accumulated_stress * 18))
        elif accumulated_stress > 2.0:
            # Stress sévère: 55-65% perte
            stress_loss_percent = max(55, min(65, 35 + accumulated_stress * 15))
        elif accumulated_stress > 1.5:
            # Stress modéré-sévère: 45-55% perte
            stress_loss_percent = max(45, min(55, 25 + accumulated_stress * 20))
        elif accumulated_stress > 1.0:
            # Stress modéré: 35-45% perte
            stress_loss_percent = max(35, min(45, 15 + accumulated_stress * 30))
        elif accumulated_stress > 0.5:
            # Stress léger-modéré: 20-35% perte
            stress_loss_percent = max(20, min(35, accumulated_stress * 40))
        else:
            # Stress très faible: 0-20% perte
            stress_loss_percent = max(0, min(20, accumulated_stress * 35))
