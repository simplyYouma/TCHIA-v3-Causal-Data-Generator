...
class FarmerDecisionModel:
    ...
    def adjust_management(
        ...
        crop_price = economic_context.get("crop_price_cfa_kg", 150)
...
class CausalCoherentGenerator:
    ...
    def _simulate_crop_cycle(
        ...
        for stage_name, duration_frac in stage_durations.items():
            ...
            if month in ["novembre", "décembre"]:
                month = "octobre"  # Approximation fin de saison
            
            # Pluie du mois avec ruissellement
            monthly_rain = monthly_rainfall.get(month, 0)
            rain_per_stage = monthly_rain * duration_days / 30
    ...
    def _calculate_coherent_yield(
        ...
        potential_yield = yield_range[1] * scenario.yield_potential
        
        # Réductions causales cumulatives
        
        # 1. Perte due aux stress accumulés (avec plafond)
        stress_loss = min(0.7, crop_state.potential_loss)
        
        # 2. Perte due à croissance réduite
        growth_loss = crop_state.growth_reduction * 0.5
        
        # 3. Bonus/malus gestion
        management_factor = (
            management["fertilizer_adjustment"] * 0.15 +  # Max +15% avec engrais
            management["density_adjustment"] * 0.10       # Max +10% avec densité
        )
        
        # 4. Facteur hydrique global
        water_sufficiency = min(1.0, cycle_data["total_effective_rain"] / 
                               self.crop_profiles.get_stress_thresholds(crop)["rainfall_mm"])
        
        # Calcul final cohérent
        actual_yield = potential_yield * (
            (1 - stress_loss) *           # Pertes irréversibles
            (1 - growth_loss) *           # Croissance réduite
            (1 + management_factor) *     # Gestion
            water_sufficiency             # Suffisance hydrique
        )
        
        # Biomasse totale (paille + grain)
        ...
        return {
            ...
            "loss_percent": round((1 - actual_yield/potential_yield) * 100, 1),
