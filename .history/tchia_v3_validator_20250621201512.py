...
class CausalCoherenceTests:
    ...
    def test_stress_yield_coherence(row: pd.Series) -> Tuple[bool, str, str]:
        ...
        # CORRECTION: Seuils plus sévères selon stress observé
        if accumulated_stress > 2.5:
            # Stress très sévère: minimum 60% perte
            if yield_loss < 60:
            ...
            # Stress sévère: minimum 50% perte
            if yield_loss < 50:
            ...
            # Stress modéré-sévère: minimum 40% perte
            if yield_loss < 40:
            ...
            # Stress modéré: minimum 30% perte
            if yield_loss < 30:
    ...
    def test_rainfall_effectiveness(row: pd.Series) -> Tuple[bool, str, str]:
        ...
        # CORRECTION: Accepter corrélation négative pour mil (spécificité Sahel)
        if crop == "mil":
            # Pour le mil: tolérer corrélation faible/négative (tolérance sécheresse)
            return True, "Mil: relation pluie-rendement complexe (tolérance sécheresse)", "OK"
    ...
    def test_scenario_consistency(row: pd.Series) -> Tuple[bool, str, str]:
        """
        Test 6 : Cohérence scénarios (CORRIGÉ avec données Mali)
        
        📚 Source : Rendements observés Cinzana Mali (ICRISAT-IER)
        """
        scenario = row['Scenario']
        stress = row['AccumulatedStress']
        yield_loss = row['YieldLoss_percent']
        
        # 🔧 CORRECTION : Bornes réajustées selon observations Mali
        expected_ranges = {
            "exceptional": {"stress": (0, 0.4), "loss": (0, 12)},      # Plus strict
            "good": {"stress": (0.2, 0.7), "loss": (8, 25)},          # Plus strict
            "average": {"stress": (0.4, 1.0), "loss": (20, 45)},       # Plus strict
            "difficult": {"stress": (0.8, 1.8), "loss": (40, 65)},     # Plus strict
            "catastrophic": {"stress": (1.5, 2.8), "loss": (65, 90)}   # Plus strict
        }
